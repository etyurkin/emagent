...
(ert-deftest emagent-prompts-test-prefer-emacs-prompt ()
  ...
  (let ((prompt (emagent-prompts--prefer-emacs-prompt)))
    ...
    (should (string-match-p "Turns and waiting" prompt))
    (should (string-match-p "ScheduleWakeup" prompt))
    (should (string-match-p "do not narrate waiting" prompt))
    ...
    (should (string-match-p "list_processes" prompt))
