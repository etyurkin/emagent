...
(defun emagent-acp-compat--session-load-with-meta (orig-fn &rest args)
  "Extend `acp-make-session-load-request' to accept an optional :meta keyword.

When `:meta' is present in ARGS, it is injected into the `_meta' field of
the request params using the same convention as `acp-make-session-new-request'.
The acp package on disk is not modified."
  (let* ((meta (plist-get args :meta))
         (filtered (cl-loop for (key val) on args by #'cddr
                            unless (eq key :meta)
                            append (list key val)))
         (req (apply orig-fn filtered)))
    (when meta
      (let ((params-pair (assq :params req)))
        (setcdr params-pair
                (append (cdr params-pair) `((_meta . ,meta))))))
    req))
...
(defun emagent-acp-compat--install ()
  ...
  (unless emagent-acp-compat--installed
    ...
    (advice-add 'acp-make-session-load-request :around
                #'emagent-acp-compat--session-load-with-meta)
    (setq emagent-acp-compat--installed t)))

(emagent-acp-compat--install)

;; Always ensure the session-load meta extension is active,
;; even if --install already ran in a previous load.
(advice-add 'acp-make-session-load-request :around
            #'emagent-acp-compat--session-load-with-meta)
