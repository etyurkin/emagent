...
`json-parse-buffer' with `:array-type 'list' yields lists, but `json-serialize'
treats lists as alists and requires symbol keys — so values like npx \"-y\" args
fail with \"Wrong type argument: symbolp\"."
  (cond
   ((hash-table-p object)
    (maphash (lambda (_key value)
               (puthash _key (emagent-mcp--lists-to-vectors value) object))
             object)
    object)
