...
(defconst emagent-mcp--tools
  (list
   ...
   (list "buffer_search"
         "Search one project file for PATTERN (includes unsaved buffer content). Use buffer_list for paths. Prefer over grep for a single open or dirty file; use grep for whole-project search. Literal strings: set literal=true. Paginate with from_line=resume_from_line from the footer. Emacs regexp syntax, not PCRE."
         '(("file" . ((type . "string")
                      (description . "File path relative to session root (required). Use buffer_list to discover open files.")))
           ("pattern" . ((type . "string")
                          (description . "Emacs regexp to search for, or a literal string when literal is true.")))
           ("from_line" . ((type . "integer")
                           (description . "1-based line to start searching (default 1).")))
           ("to_line" . ((type . "integer")
                         (description . "1-based line to stop searching (default: end of file).")))
           ("limit" . ((type . "integer")
                       (description . "Maximum matches to return (default 20, max 50).")))
           ("literal" . ((type . "boolean")
                         (description . "When true, treat pattern as a literal string (search-forward).")))
           ("ignore_case" . ((type . "boolean")
                             (description . "Case-fold search (default false).")))
           ("backward" . ((type . "boolean")
                          (description . "Search backward from to_line (default false).")))
           ("context" . ((type . "integer")
                         (description . "Lines of context before/after each match (0–3, default 0)."))))
         '("file" "pattern")
         (lambda (args)
           (emagent-tool-buffer-search
            (emagent-mcp--arg args "file")
            (emagent-mcp--arg args "pattern")
            (emagent-mcp--arg args "from_line")
            (emagent-mcp--arg args "to_line")
            (emagent-mcp--arg args "limit")
            (emagent-mcp--bool args "literal")
            (emagent-mcp--bool args "ignore_case")
            (emagent-mcp--bool args "backward")
            (emagent-mcp--arg args "context"))))
   (list "find_files"
