...
context, thinking, …).  Other agents still use bracketed model ids and/or
global =model_config= / =thought_level= rows.  Type part of a model name to
narrow the list.  Reconnect the session after upgrading so Cursor picks up
the capability.
