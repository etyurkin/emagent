...
(before the first =* user>= heading, or in an Org setup file).  On send,
={{{name}}}= and ={{{name(args)}}}= expand into the macro body; the buffer
text is left unchanged.  Undefined macros abort the send.  A macro body may
include a per-turn =[[emagent://…]]= model link (same marker =/model=
inserts).
