...
emagent estimates fill from MCP payload bytes plus a capped Org transcript
term (=emagent-acp-ctx-proxy-buffer-cap=, default 5% of the proxy window)
against =emagent-acp-ctx-proxy-size=, and shows =ctx:~N%=.  When fill is high, a
...
| =emagent-acp-ctx-proxy-buffer-cap= | =0.05= | Max transcript share of that window |
