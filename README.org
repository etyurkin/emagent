...
**/model=** inserts an Org link
(=[[emagent://AGENT/MODEL][short]]=).  Emagent runs that turn on the chosen
model, shows =** Thinking (model)=, then restores the buffer model.  Delete the
link to cancel.  On failure, emagent asks whether to keep it for retry/continue.
...
- *Project notes*: =.emagent/notes.org= under the project root
  (=emagent-project-notes-relative=), merged into the same prompt block so
  =/compact= does not keep re-summarizing repo facts.
...
Claude reports real context fill as =ctx:N%=.  Cursor often omits usage; emagent
estimates from MCP payload bytes + buffer size against
...
- =** Thinking= — reasoning and tool lines (=→ tool: path=).  Folds when the
  turn finishes.  Shell/=eval=/CLI tools render as Org src blocks with
...
Context fill uses the =ctx:~N%= proxy when the agent omits usage (see above).
...
| =emagent-project-notes-relative= | =.emagent/notes.org= | Per-project notes file |
