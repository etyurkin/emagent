...
Define reusable prompt snippets with Org =#+MACRO:= in the session preamble
(before the first =* user>= heading, or in an Org setup file so every
session inherits them).  On send, ={{{name}}}= and ={{{name(args)}}}=
expand into the macro body; the buffer text is left unchanged.  Undefined
macros abort the send.

A macro body may include a per-turn =[[emagent://AGENT/MODEL][label]]=
model link (same marker =/model= inserts).  Emagent scans the /expanded/
text, switches for that turn only, then strips the link before the agent
sees the prompt.  Pick =AGENT/MODEL= from =/model TAB= — ids vary by
provider.  If the model is unavailable or the switch fails, the send
aborts (it does not fall through to the session model).

Macros do /not/ rewrite free text.  Typing an ordinary word does
nothing unless you defined a macro with that name and invoke it as
={{{name}}}= (or paste a model link yourself).

*** Example: cheap model for a chore

Org =#+MACRO:= bodies are a single line (use =$1=, =$2=, … for args).
Put definitions in the preamble or a shared Org setup file:

#+begin_example
#+MACRO: haiku [[emagent://claude/claude-haiku-4-5][haiku]]
#+MACRO: commit [[emagent://claude/claude-haiku-4-5][haiku]] Commit the current work. Follow the repo commit rules; do not push.
#+MACRO: commit1 [[emagent://claude/claude-haiku-4-5][haiku]] Commit with subject hint: $1. Follow the repo commit rules; do not push.
#+end_example

Then send a prompt that invokes the macro (extra lines after ={{{…}}}= are fine — only the macro token expands):

#+begin_example
{{{haiku}}} summarize the last diff in three bullets
#+end_example

#+begin_example
{{{commit}}}
only the staged changes on this branch
#+end_example

#+begin_example
{{{commit1(Fix compile retry on detached builds)}}}
#+end_example
...
=/model= and =M-x emagent-set-model= offer a flat, filterable list of every
advertised variant.  For Cursor, emagent advertises
=_meta.parameterizedModelPicker= and expands each model with its own
parameter selects from =cursor/list_available_models= (effort, fast,
context, thinking, …).  For Claude, whose adapter only reports effort
levels for the current model, emagent probes each model once at connect
\(switching the idle session through the model list and restoring it) so
every row shows exactly the effort levels that model supports.  Other
agents use bracketed model ids and/or global =model_config= /
=thought_level= rows.  A row's bracket tags show only what it turns on:
off/false/default values are applied (and persisted) but hidden, so the
bare row is the model at its plainest settings — e.g. =grok-4.5 [Low]=
sets =effort=low, fast=false= while =grok-4.5 [Low] [Fast]= sets
=fast=true=.  The echo area states the full applied spec after a switch.
Type part of a model name to narrow the list.  Reconnect the session
after upgrading so Cursor picks up the capability.

=M-x emagent= reuses the project's existing session buffer instead of
creating a second one; with an existing buffer the picker is limited to
that buffer's agent (a session cannot change agents), and picking a
variant applies it to the live session.
=/model= inserts an Org link (=[[emagent://AGENT/MODEL][label]]=).  Emagent
...
Save the buffer (=C-x C-s=), or rely on autosave: file-backed sessions save
after each finished turn and after idle (=emagent-session-autosave=, default
on).  Pathless scratch buffers get a file under =~/.emagent/scratch/= on the
first agent send (not on client slash commands), then the same autosave
applies.  Files start with =# -*- mode: emagent -*-= and restore
...
(=emagent-acp-ctx-proxy-buffer-divisor= chars/token), mapped with a soft
curve so =ctx:~N%= grows over a session without jumping past 100%.  When
fill is high, a
...
Prose like "Waiting…" or "Checking progress…" does /nothing/ — tool calls
block until they finish (or time out).  Long builds should use =shell
op=compile= with a large enough =timeout=, or an explicit =ScheduleWakeup=;
the system prompt tells the agent not to narrate idle waits.
...
Emagent advertises a small set of MCP tools.  Families like =fs=, =git=,
=shell=, =elisp=, and =structural= take an =op= argument; session/UI tools are
typed top-level names that return wide JSON records (filters + =limit=).
Full op docs live behind =elisp= =op=guide= / =describe= when needed
(=emagent-mcp-compact-schemas= keeps =tools/list= short).

| Tool | Ops / role |
|------+------------|
| =fs= | =read= =write= =edit= =undo= =delete= =delete_directory= =list= =find= (write/edit/undo/delete need =expected_tick=; Lisp → =structural=) |
| =search= | Regexp search over the session tree (=rg= when available) |
| =git= | =status= =diff= =log= |
...
| =project_directory= / =where_is= | Session root path / key bindings for a command |
| =list_buffers= / =buffer_info= | Filterable buffer records / one-buffer detail |
| =list_windows= / =list_frames= | Live window and frame layout |
| =list_marks= / =list_registers= / =list_bookmarks= | Marks, registers, bookmarks |
| =list_diagnostics= | Flymake diagnostics |
| =list_processes= | Emacs subprocesses and network processes |
| =imenu_index= | Buffer outline (any major mode with imenu) |
...
| =emagent-acp-ctx-proxy-buffer-divisor= | =40= | Transcript chars per estimated token |
...
| =emagent-session-autosave= | =t= | Idle + turn-end save for visited files |
| =emagent-session-autosave-idle-seconds= | =30= | Idle seconds before visited-file save |
| =emagent-session-scratch-directory= | =~/.emagent/scratch= | Default path for pathless sessions |
...
| =emagent-chat-undo-limit= | =65536= | Soft undo cap (bytes) in chat buffers |
