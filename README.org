...
advertised variant (Cursor bracket ids such as =[context=200k]= /
=[fast=true]=, and composed =model_config= / =thought_level= rows when the
agent exposes those).  Type part of a model name to narrow the list.
