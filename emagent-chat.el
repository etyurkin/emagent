...
(defvar emagent-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-c C-v") #'emagent-set-model)
...
(defun emagent-chat--command-matches-needle-p (name needle)
  "Return non-nil when slash command NAME matches partial NEEDLE."
  (if (string-empty-p needle)
      t
    (let ((base (emagent-chat--command-needle-base needle)))
      (or (string-prefix-p needle name)
          (string-prefix-p base name)
          (string-prefix-p base (emagent-chat--command-skill-part name))
          (cl-search needle name :test #'char-equal)
          (cl-search base name :test #'char-equal)))))
